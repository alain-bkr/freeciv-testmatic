...
#  or pick a random value if needed
...
DIR="Sz${size}-Ai${aifill}-Lm${land}-$now"	# 30 char max
...
####################################################
# the name of our generated file	=> we can begin to fill our .serv file with Serv function
name="$DIR.serv"
...
Serv "set savename $(basename $name .serv)-T%04T-%R"
...
set scorefile $(basename $name .srv).score
...
gen=(RANDOM FRACTAL ISLAND FRACTURE FAIR)
...
# ignore  1..100 %,  go for  100..10000 %
Serv "set globalwarming_percent $(R 100-10000)"

Serv "set nuclearwinter enabled"
# ignore  1..100 %,  go for  100..10000 %
Serv "set nuclearwinter_percent $(R 100-10000)"
...
hap=( DISABLED NATIONAL ALLIED)
h=$(R 0-2)
Serv "set happyborders ${hap[$h]}"

list=( DISABLED NORMAL FREQUENT HORDES)
l=$(R 0-3)
Serv "set barbarians ${list[$l]}"

Serv "set onsetbarbs $(R 0-120)"

list=( 25 50 100 200 400)
l=$(R 0-4)
Serv "set sciencebox ${list[$l]}"

#  phasemode	(ALL) PLAYER TEAM
list=( ALL PLAYER TEAM)
l=$(R 0-2)
...
Serv "# If we want only one ai, just append onaAI.srv on the command line of $0"
Serv "# Be sure to have at least 2 teams, AI*1 is alone in its own team."
Serv "team AI*1 1"

# stronger first, to bias toward stronger AI (random pick = team %6 , so for example in case of 9 AI, cheating have more chance (2/9) than novice (1/9) )
ait=( cheating experimental hard normal easy novice)
j=$(R 0-5)
Serv "${ait[$j]} AI*1" 
...
for f in $INCLUDES; do
	Serv "##########################"
	Serv "# include file $f"
	# do NOT include the 'start' command, neither other stuff that will be added later
	cat $f|grep -v -e "^start$" -e "^save$" -e "^mapimg define" >>$DIR/$name
done 

##########################  Finish the .srv file with autogames additions , and if desired map image generation ########
...
# image can be done only after mapgeneration, but could be defined earlier (before 'start') 
#  by putting this line after 'start' we have an image a turn 1 , then t10 t20 ...
# (i fail to have ImageMagic detected when i compile with clang... so just use ppm built-in format to be sure)
if [ "$IMG" != "0" ]; then
...
ls -l $DIR/$name
echo " $DIR/$name has been created"
...
(
	...
	if [ -f gmon.out ]; then
		cp -a gmon.out gmon.$(basename $name .serv).out
	fi
	# quick search for error messages
	cat console.log |grep -i -e error -e "^1" -e saved |grep -B1 -i -e error -e "^1" > game.error
	cat serv.log | grep "^1" > serv.log.error

	# save space, convert 1MB Sz05-Ai031-Lm27-20230330_172315-T01234-M-bc--tuZ1Pall.map.ppm to several kB T01234.png
	if [ "$IMG" != "0" ] && [ $(which convert) ]; then
		for f in $(ls *.ppm) ; do
			im=$( echo $f |cut -d- -f-5)
			convert $f $im.png && rm $f
		done
		## should make an animated gif	# display with  ` animate xyz.gif `
		# convert -delay 10 -loop 0 *.png $(echo $f |cut -d- -f-4).gif
	fi
